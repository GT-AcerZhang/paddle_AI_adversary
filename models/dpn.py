...
class DPN(object):
    def __init__(self, layers=68, prefix=''):
        self.layers = layers
        self.prefix = prefix
    ...
    def net(self, input, class_dim=1000):
        ...
        conv1_x_1 = fluid.layers.conv2d(
            ...
            act=None,
            bias_attr=False,
            name=self.prefix + "conv1",
            param_attr=ParamAttr(name=self.prefix + "conv1_weights"), )

        conv1_x_1 = fluid.layers.batch_norm(
            input=conv1_x_1,
            act='relu',
            is_test=False,
            name=self.prefix + "conv1_bn",
            param_attr=ParamAttr(name=self.prefix + 'conv1_bn_scale'),
            bias_attr=ParamAttr(self.prefix + 'conv1_bn_offset'),
            moving_mean_name=self.prefix + 'conv1_bn_mean',
            moving_variance_name=self.prefix + 'conv1_bn_variance', )
        ...
        conv5_x_x = fluid.layers.batch_norm(
            ...
            act='relu',
            is_test=False,
            name=self.prefix + "final_concat_bn",
            param_attr=ParamAttr(name=self.prefix + 'final_concat_bn_scale'),
            bias_attr=ParamAttr(self.prefix + 'final_concat_bn_offset'),
            moving_mean_name=self.prefix + 'final_concat_bn_mean',
            moving_variance_name=self.prefix + 'final_concat_bn_variance', )
        ...
        fc6 = fluid.layers.fc(input=pool5,
                              size=class_dim,
                              param_attr=ParamAttr(initializer=fluid.initializer.Uniform(-stdv, stdv), name=self.prefix + 'fc_weights'),
    ...
    def dual_path_factory(self,
        ...
        if has_proj:
            ...
            data_o1, data_o2 = fluid.layers.split(
                ...
                dim=1,
                name=self.prefix + name + "_match_conv_Slice")
        else:
            data_o1 = data[0]
            data_o2 = data[1]
        ...
        c1x1_c1, c1x1_c2 = fluid.layers.split(
            ...
            dim=1,
            name=self.prefix + name + "_conv3_Slice")

        # OUTPUTS
        summ = fluid.layers.elementwise_add(
            x=data_o1, y=c1x1_c1, name=self.prefix + name + "_elewise")
        dense = fluid.layers.concat(
            [data_o2, c1x1_c2], axis=1, name=self.prefix + name + "_concat")
...
def DPN68(prefix=''):
    model = DPN(layers=68, prefix=prefix)
    return model


def DPN92(prefix=''):
    model = DPN(layers=92, prefix=prefix)
    return model


def DPN98(prefix=''):
    model = DPN(layers=98, prefix=prefix)
    return model


def DPN107(prefix=''):
    model = DPN(layers=107, prefix=prefix)
    return model


def DPN131(prefix=''):
    model = DPN(layers=131, prefix=prefix)
    return model