...
class EfficientNet():
    def __init__(self, name='b0', padding_type='SAME', override_params=None, is_test=False, use_se=True, prefix=''):
    ...
    def net(self, input, class_dim=1000, is_test=False):
        ...
        if self._global_params.dropout_rate:
            pool = fluid.layers.dropout(pool, self._global_params.dropout_rate,
                                        dropout_implementation='upscale_in_train')

        param_attr, bias_attr = init_fc_layer(class_dim, self.prefix + '_fc')
        out = fluid.layers.fc(pool, class_dim, name=self.prefix + '_fc', param_attr=param_attr, bias_attr=bias_attr)
        return out
    ...
    def se_block(self, inputs, num_squeezed_channels, oup, name):
        ...
        x_squeezed = conv2d(x_squeezed,
                            ...
                            padding_type=self.padding_type,
                            act='swish',
                            name=self.prefix + name + '_se_reduce')
        x_squeezed = conv2d(x_squeezed,
                            num_filters=oup,
                            filter_size=1,
                            use_bias=True,
                            padding_type=self.padding_type,
                            name=self.prefix + name + '_se_expand')
...
def EfficientNetB0(is_test=False, padding_type='SAME', override_params=None, use_se=True, prefix=''):
    model = EfficientNet(name='b0', is_test=is_test, padding_type=padding_type, override_params=override_params,
                         use_se=use_se, prefix=prefix)
    return model


def EfficientNetB1(is_test=False, padding_type='SAME', override_params=None, use_se=True, prefix=''):
    model = EfficientNet(name='b1', is_test=is_test, padding_type=padding_type, override_params=override_params,
                         use_se=use_se, prefix=prefix)
    return model


def EfficientNetB2(is_test=False, padding_type='SAME', override_params=None, use_se=True, prefix=''):
    model = EfficientNet(name='b2', is_test=is_test, padding_type=padding_type, override_params=override_params,
                         use_se=use_se, prefix=prefix)
    return model


def EfficientNetB3(is_test=False, padding_type='SAME', override_params=None, use_se=True, prefix=''):
    model = EfficientNet(name='b3', is_test=is_test, padding_type=padding_type, override_params=override_params,
                         use_se=use_se, prefix=prefix)
    return model


def EfficientNetB4(is_test=False, padding_type='SAME', override_params=None, use_se=True, prefix=''):
    model = EfficientNet(name='b4', is_test=is_test, padding_type=padding_type, override_params=override_params,
                         use_se=use_se, prefix=prefix)
    return model


def EfficientNetB5(is_test=False, padding_type='SAME', override_params=None, use_se=True, prefix=''):
    model = EfficientNet(name='b5', is_test=is_test, padding_type=padding_type, override_params=override_params,
                         use_se=use_se, prefix=prefix)
    return model


def EfficientNetB6(is_test=False, padding_type='SAME', override_params=None, use_se=True, prefix=''):
    model = EfficientNet(name='b6', is_test=is_test, padding_type=padding_type, override_params=override_params,
                         use_se=use_se, prefix=prefix)
    return model


def EfficientNetB7(is_test=False, padding_type='SAME', override_params=None, use_se=True, prefix=''):
