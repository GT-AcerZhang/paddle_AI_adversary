...
class HRNet():
    def __init__(self, width=18, has_se=False, prefix=''):
        self.width = width
        self.has_se = has_se
        self.prefix = prefix
        self.channels = {
    ...
    def net(self, input, class_dim=1000):
        ...
        out = fluid.layers.fc(input=pool, size=class_dim,
                              param_attr=ParamAttr(name=self.prefix + 'fc_weights',
                                                   initializer=fluid.initializer.Uniform(-stdv, stdv)),
                              bias_attr=ParamAttr(name=self.prefix + 'fc_offset'))
        return out
                    ...
                    for k in range(i - j):
                        if k == i - j - 1:
                            y = self.conv_bn_layer(y, filter_size=3, num_filters=channels[i], stride=2, if_act=False,
                                                   name=name + '_layer_' + str(i + 1) + '_' + str(j + 1) + '_' + str(
                                                       k + 1))
                        else:
    ...
    def last_cls_out(self, x, name=None):
        ...
        for i in range(len(x)):
            out.append(
                self.bottleneck_block(input=x[i], num_filters=num_filters_list[i], name=name + 'conv_' + str(i + 1),
                                      downsample=True))
    ...
    def basic_block(self, input, num_filters, stride=1, downsample=False, name=None):
        residual = input
        conv = self.conv_bn_layer(input=input, filter_size=3, num_filters=num_filters, stride=stride,
                                  name=name + '_conv1')
        conv = self.conv_bn_layer(input=conv, filter_size=3, num_filters=num_filters, if_act=False,
                                  name=name + '_conv2')
        if downsample:
    ...
    def bottleneck_block(self, input, num_filters, stride=1, downsample=False, name=None):
        ...
        conv = self.conv_bn_layer(input=conv, filter_size=3, num_filters=num_filters, stride=stride,
                                  name=name + '_conv2')
    ...
    def squeeze_excitation(self, input, num_channels, reduction_ratio, name=None):
        ...
        squeeze = fluid.layers.fc(input=pool,
                                  ...
                                  param_attr=fluid.param_attr.ParamAttr(
                                      initializer=fluid.initializer.Uniform(
                                          -stdv, stdv), name=self.prefix + name + '_sqz_weights'),
                                  bias_attr=ParamAttr(name=self.prefix + name + '_sqz_offset'))
        ...
        excitation = fluid.layers.fc(input=squeeze,
                                     ...
                                     param_attr=fluid.param_attr.ParamAttr(
                                         initializer=fluid.initializer.Uniform(
                                             -stdv, stdv), name=self.prefix + name + '_exc_weights'),
                                     bias_attr=ParamAttr(name=self.prefix + name + '_exc_offset'))
    ...
    def conv_bn_layer(self, input, filter_size, num_filters, stride=1, padding=1, num_groups=1, if_act=True, name=None):
        conv = fluid.layers.conv2d(
            ...
            groups=num_groups,
            act=None,
            param_attr=ParamAttr(initializer=MSRA(), name=self.prefix + name + '_weights'),
            bias_attr=False)
        bn_name = self.prefix + name + '_bn'
...
def HRNet_W18_C(prefix=''):
    model = HRNet(width=18, prefix=prefix)
    return model


def HRNet_W30_C(prefix=''):
    model = HRNet(width=30, prefix=prefix)
    return model


def HRNet_W32_C(prefix=''):
    model = HRNet(width=32, prefix=prefix)
    return model


def HRNet_W40_C(prefix=''):
    model = HRNet(width=40, prefix=prefix)
    return model


def HRNet_W44_C(prefix=''):
    model = HRNet(width=44, prefix=prefix)
    return model


def HRNet_W48_C(prefix=''):
    model = HRNet(width=48, prefix=prefix)
    return model


def HRNet_W60_C(prefix=''):
    model = HRNet(width=60, prefix=prefix)
    return model


def HRNet_W64_C(prefix=''):
    model = HRNet(width=64, prefix=prefix)
    return model


def SE_HRNet_W18_C(prefix=''):
    model = HRNet(width=18, has_se=True, prefix=prefix)
    return model


def SE_HRNet_W30_C(prefix=''):
    model = HRNet(width=30, has_se=True, prefix=prefix)
    return model


def SE_HRNet_W32_C(prefix=''):
    model = HRNet(width=32, has_se=True, prefix=prefix)
    return model


def SE_HRNet_W40_C(prefix=''):
    model = HRNet(width=40, has_se=True, prefix=prefix)
    return model


def SE_HRNet_W44_C(prefix=''):
    model = HRNet(width=44, has_se=True, prefix=prefix)
    return model


def SE_HRNet_W48_C(prefix=''):
    model = HRNet(width=48, has_se=True, prefix=prefix)
    return model


def SE_HRNet_W60_C(prefix=''):
    model = HRNet(width=60, has_se=True, prefix=prefix)
    return model


def SE_HRNet_W64_C(prefix=''):
    model = HRNet(width=64, has_se=True, prefix=prefix)
    return model